...
# Fields containing sensitive data that should be redacted from logs
REDACTED_FIELDS = ["key"]
...
class ModelsLabImageGenerationConfig(BaseImageGenerationConfig):
                    ...
                    if k == "n":
                        ...
                        if "x" in size_str:
                            ...
                            try:
                                ...
                                raise ValueError(
                                    f"Invalid size values in '{size_str}'. "
                                    f"Width and height must be integers (e.g., '512x512')"
                                )
    ...
    def transform_image_generation_response(
        ...
        # Handle async processing: poll until completion
        if status == "processing":
            ...
            # Poll for completion - _poll_sync returns data with status="success"
            response_data = self._poll_sync(
                generation_id=generation_id,
                api_key=resolved_key,
                base_url=base_url,
            )
            # After polling, response_data has status="success"

        # Handle successful response (either direct or after polling)
